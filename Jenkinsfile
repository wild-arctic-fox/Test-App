    ...
    stages {
        ...
        stage("deploy") {
            steps {
                script {
                    gv.deployApp()
                    def docker_kill = 'docker kill $(docker ps -q)'
                    def docker_cmd = 'docker run -d -p 3099:3000 wildarcticfox/wild-private-repo:$IMAGE_NAME'
                    sshagent(['ec2-server-name']) {
                        sh "ssh -o StrictHostKeyChecking=no ec2-user@54.146.71.196 ${docker_kill}"
